...
- check and claim files before writing, and what "your own" claim means
- hand a session off with `peek at <selector> --mode handoff --out <file>`, and over MCP
  write the document from `material`
- read `--json` errors and exit codes
