...
- `peek_session` — read one session snapshot. `mode=brief` or `structured` for "what
  is that agent doing"; `mode=handoff` returns `material`, a complete prompt with the
  compressed transcript, which the calling agent answers itself to produce the
  handoff document (see [Handoff](/handoff)). Nothing is spawned.
...
- `agent-peek://session/{selector}/handoff` — handoff material for one session; the caller writes the document.
...
:::note
The server writes its registry, claims and usage index under `~/.agent-peek`. A
client sandbox that forbids that makes every tool fail with `state_unwritable`;
allow writes to that directory.
:::
